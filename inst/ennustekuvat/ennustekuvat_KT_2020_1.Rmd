...
indeksoi <- function(d, vuosi = 2020){
  bkt_avg_vuosi <- d |> 
    filter(year(time) == vuosi) |> 
    pull(value) |> 
    mean()
  d |> 
    mutate(value = (value/bkt_avg_vuosi) * 100)
} 

usa_bkt <- ptt_data_robo("fred/GDPC1") |> 
...
suomi_bkt <- ptt_data_robo_l("StatFin/kan/ntp/statfin_ntp_pxt_132h.px") |>
  filter(
    taloustoimi == "B1GMH Bruttokansantuote markkinahintaan",
...
euroalue_bkt <- ptt_data_robo_l("eurostat/namq_10_gdp") %>% 
...
yhdistetty_bkt_data|>
  ptt_plot(
    ...
    caption =  "Lähde: Tilastokeskus, Fred, Eurostat ja PTT",
    rangeslider = FALSE,
    ## rangeslider = "2015-01-01",
    zeroline = T,
...
```{r}
vaihtotase <- ptt_data_robo_l("StatFin/kan/mata/kk/statfin_mata_pxt_114s.px") %>% 
  filter(maksutase_era == "CA Vaihtotase", 
         tiedot == "Netto, miljoonaa euroa",
        time >= "2014-01-01",
        time < "2022-01-01"
) %>% 
  mutate(year = year(time)) 

# tarkastetaan, että ei ole vajaita vuosia
# vaihtotase %>% group_by(year) %>% count() # arvoja joka vuosi 12, ok.

vaihtotase_summarized <- vaihtotase %>% group_by(year) %>% summarize(value = sum(value)) %>% 
  mutate(tiedot = "vaihtotase") %>% mutate(time = as_date(paste0(year, "-07-01"))) %>% select(tiedot, time, value) 

kauppatase <- ptt_data_robo_l("StatFin/kan/mata/kk/statfin_mata_pxt_12gf.px") %>% 
  filter(tiedot == "Tulojen ja menojen erotus (netto), miljoonaa euroa",
         maksutase_era == "G Tavarat",
         time >= "2014-01-01",
         time < "2022-01-01") %>% 
  mutate(year = year(time))

# tarkastetaan että ei ole vajaita vuosia
# kauppatase %>% group_by(year) %>% count() # arvoja joka vuosi 12, ok.

kauppatase_summarized <- kauppatase %>% group_by(year) %>% summarize(value = sum(value)) %>% 
  mutate(tiedot = "kauppatase") %>% mutate(time = as_date(paste0(year, "-07-01"))) %>% select(tiedot, time, value)
  

rbind(vaihtotase_summarized, kauppatase_summarized) %>% 
  mutate(value = value / 1000) %>% # muunnos miljardeihin
  ptt_plot(
    grouping=tiedot, 
