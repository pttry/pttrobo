...
vaihtotase_summarized <- vaihtotase %>% group_by(year) %>% summarize(value = sum(value)) %>% 
  mutate(tiedot = "vaihtotase") %>% mutate(time = as_date(paste0(year, "-01-01"))) %>% select(tiedot, time, value) 
...
kauppatase_summarized <- kauppatase %>% group_by(year) %>% summarize(value = sum(value)) %>% 
  mutate(tiedot = "kauppatase") %>% mutate(time = as_date(paste0(year, "-01-01"))) %>% select(tiedot, time, value)
...
rbind(vaihtotase_summarized, kauppatase_summarized) %>% 
  ...
  ptt_plot(
    ...
    axis_limits = list(time = c(as_date("2014-01-01"),as_date("2017-01-01")))
  ) 
