...
## Työttömyys
```{r}
tyottomyysaste_plot <- ptt_data_robo_l("StatFin/tym/tyti/kk/statfin_tyti_pxt_135z.px") |> # työttömyysaste
  filter(
    tiedot == "Työttömyysaste, %, trendi",
    time >= "2014-01-01"
  ) |>
  mutate(tiedot = "Työttömyysaste") |>
# yhdistetään tem työttömyysaste dataan (tähän ei ole ennustetta)
rbind(
  ptt_data_robo_l("StatFin/tym/tyonv/kk/statfin_tyonv_pxt_12tf.px") |>
...
# yhdistetään
rbind(tyottomyysaste, tyottomyysaste_tem) |>
plotly::add_trace(data=palkansaajat, y = ~value,
                  text = "Palkansaajat",
                  type = "scatter", mode="lines",
                  line = list(width = 4),
                  color = "#C36B0D",
                  hovertemplate = pttrobo:::ptt_plot_hovertemplate(
                    list(rounding = 1, unit = "%", extra = "", timeformat = attributes(d)$frequency$en)),
                  name="Työttömyysaste (TEM)")
