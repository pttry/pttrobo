...
```{r storage_setup, include=FALSE, eval=FALSE}
googleCloudStorageR::gcs_auth(Sys.glob("robottiperhe-*.json"))
googleCloudStorageR::gcs_global_bucket("pttry")
```
...
ptt_data_robo("StatFin/kan/tpulk/statfin_tpulk_pxt_12gq.px") |>
  ...
  filter(time >= "2014-01-01") |> 
  spread(alue, value) |> 
  mutate(value = 100 * RU / YHT) |> mutate(year = lubridate::year(time)) |> select(-time) |> group_by(year, tiedot, palveluera) |> summarise(value = mean(value)) |> arrange(year) 
    ptt_plot(
    grouping = tiedot,
