...
data_kv_hinta <- map(set_names(c("kuusitukki", "mäntykuitu", "kuusikuitu", "mäntytukki")), 
                     ~readxl::read_xlsx("~/../Pellervon Taloustutkimus PTT ry/Ennuste - Taulut Metsä/kv_puunhinta.xlsx", skip = 1)|> 
...
ptt_data_robo_l("luke/04_Metsa/04_Talous/16_Kantorahatulot/01_Bruttokantorahat_v_omistajaryhma.px") |> 
  filter_recode(
    omistajaryhma = c("Kaikki yhteensä"), 
  ...
  ptt_plot_add_prediction(filter(ennuste_data), n_obs = 3) |>
  ptt_plot_create_widget()
...
data_kv_hinta[["kuusitukki"]] |> 
  ptt_plot(
    ...
    subtitle = "Indeksi, 2015 = 100",
    caption =  "Lähde: ... ja PTT",
    rangeslider = FALSE,
    zeroline = F,
    hovertext =  list(rounding = 1, unit = "", extra = "")
  ) |>
